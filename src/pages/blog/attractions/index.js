...
const Index = ({ data }) => {
  return (
    ...
    >
      <HeroComponent
        gImage={
          data.allContentfulBlogLayout.nodes[0].attractionsBlogImage.gatsbyImage
        }
        heroText="Attraction Blogs"
        button={false}
      />
      <TextComponent
        title="Blog Posts"
...
export const query = graphql`
  query MyQuery {
    ...
    allContentfulBlogPost(
      filter: { category: { eq: "Attractions" } }
      sort: { createdAt: DESC }
    ) {
      nodes {
        title
...
export const Head = ({ data }) => {
  const { title, description } = categoryBySlug("attractions");
  return (
    <>
      <Seo title={title} description={description} />
      <link
        rel="canonical"
        href="https://puntacanatourstore.com/blog/attractions/"
      />
    </>
  );
};
