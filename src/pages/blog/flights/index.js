...
const Index = ({ data }) => {
  return (
    ...
    >
      <HeroComponent
        gImage={
          data.allContentfulBlogLayout.nodes[0].flightsBlogImage.gatsbyImage
        }
        heroText="Flights Blogs"
        button={false}
      />
      <TextComponent
        title="Blog Posts"
        ...
        paragraph={
          data.allContentfulBlogLayout.nodes[0].flightsBlogDescription
            .flightsBlogDescription
        }
...
export const Head = ({ data }) => {
  const { title, description } = categoryBySlug("flights");
  return (
    <>
      <Seo title={title} description={description} />
      <link
        rel="canonical"
        href="https://puntacanatourstore.com/blog/flights/"
      />
    </>
  );
};
