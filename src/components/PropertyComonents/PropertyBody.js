...
const PropertyBody = ({ context }) => {
  ...
  const propertyDocument = documentToReactComponents(
    JSON.parse(context.raw),
    options,
  );
  return (
    <>
      <div className="flex flex-col lg:pr-5 mx-5 lg:mx-0 max-w-6xl lg:w-7/12 justify-center">
        {propertyDocument}
      </div>
    </>
  );
