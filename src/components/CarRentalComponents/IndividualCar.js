...
const IndividualCar = ({ car }) => {
  const [showMore, setShowMore] = useState(false);
  const image = getImage(car.carImage.gatsbyImage);
  return (
    <div className="max-w-sm min-w-[20rem] xl:max-w-xs my-5 mx-2 rounded-2xl overflow-hidden shadow-lg bg-gray-50">
      ...
      {showMore ? (
        <>
          ...
          <div className="flex flex-col py-5 px-2 gap-y-1 text-xs">{car.liabilityStatement.liabilityStatement}</div>
        </>
      ) : (
        <></>
      )}
